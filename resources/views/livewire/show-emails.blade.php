<div>	
    ...
    <div class="bg-white min-w-0 flex-1 px-32 mb-5">
        <template x-for="(email, index) in {{ json_encode($emails->toArray()['data']) }}" :key="index">
            <div class="border-b border-gray-100 p-5 cursor-pointer" x-on:click="current = email; showModal = true">
                <h2 class="font-medium text-l" x-text="email.to"></h2>
                Subject: <span x-text="email.subject"></span><br>
                Sent: <span x-text="email.created_at"></span><br>
                Status: <span x-text="email.mailgun_status"></span>
            </div>
        </template>
    </div>
